...
public class MessageRepository(DataContext context, IMapper mapper) : IMessageRepository
{
    ...
    public async Task<PagedList<MessageDto>> GetMessagesForUser(MessageParamas messageParamas)
    {
        ...
        {
            "Inbox" => query.Where(x => x.Recipient.UserName == messageParamas.Username 
                && x.RecipientDeleted == false),
            "Outbox" => query.Where(x => x.Sender.UserName == messageParamas.Username
                && x.SenderDeleted == false),
            _ => query.Where(x => x.Recipient.UserName == messageParamas.Username 
                && x.DateRead == null && x.RecipientDeleted == false)
        };

        var messages = query.ProjectTo<MessageDto>(mapper.ConfigurationProvider);

        return await PagedList<MessageDto>.CreateAsync(messages, messageParamas.PageNumber,
            messageParamas.PageSize);
    }
